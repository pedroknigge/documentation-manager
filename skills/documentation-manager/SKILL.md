...
description: >
  v2.5.21 — Use when bootstrapping, completing, updating, or auditing project documentation,
  ...
  "pstack", /documentation-manager. Intents: integrate, audit, from-zero,
  ...
  provenance; vibe-proof HITL; pstack bake + HITL. On conflict code wins.
license: MIT
metadata:
  author: pedroknigge
  version: "2.5.21"
...
20. **v2.0 package.** Completes the 10× line: capabilities 1.4–1.7 plus [docs/adoption-matrix.md](../../docs/adoption-matrix.md) tracking. Baseline install floor remains **2.0.0**; polyglot **2.1.0**; monorepo **2.2.0**; team **2.3.0**; Bridge complete **2.4.0**; Knowledge OS first increment **2.5.0**; current line is **2.5.21+** (catch-up on that floor — living claims + CI audit + go/no-go trail + Appendix A + §2 Mínimo + production-harden DoD + Sólido states/transitions + cold-agent readable + plans creator folder + archive-on-finish + provenance grouping + vibe-proof HITL plan bridge + pstack bake + HITL plan bridge; 2.5.13 glance; 2.5.14 `--base` HITL; 2.5.16 second dogfood ledger; 2.5.17 cargo-primary gap; 2.5.18 third dogfood ledger; 2.5.20 fourth dogfood ledger; 2.5.21 go.work stack signal; not a second 10×).
...
34. **Pstack plan bridge (v2.5.19).** Bake + HITL sibling — do **not** merge products; do **not** vendor pstack. In **plan mode**, always apply portable bake (subtract-before-add, attack the premise, falsifiable acceptance, verifiable next actions, ≥2 approaches on one-way doors, experience/outcome first). When host skills named `architect` / `figure-it-out` / a pstack plugin are present: **one pstack HITL propose** to run `/architect` (or `/figure-it-out` for large ambiguous work) **with checkpoint**, then fold into Acceptance / MVP / Next actions / Open questions. Absent → `skip:absent`. **no silent auto-run.** Cold-agent §19 stays. See [pstack-bridge.md](references/pstack-bridge.md) and [modes.md §22](references/modes.md#22-pstack-plan-bridge).
...
- “pstack” / “/architect” / “/figure-it-out” **in a plan session** → **Pstack plan bridge** ([pstack-bridge.md](references/pstack-bridge.md); modes §22): bake always; one HITL propose when present; never silent auto-run
...
Plan write/refresh: **pstack bake** always (subtract-before-add; attack the premise; falsifiable acceptance; verifiable next actions; ≥2 approaches on one-way doors; experience/outcome first). Live path: one **pstack HITL** propose when `architect` / `figure-it-out` / pstack is present — `/architect` or `/figure-it-out` **with checkpoint**; `skip:absent` when missing. **no silent auto-run.** Fold into Acceptance / MVP / Next actions / Open questions — not a second SSOT. See [pstack-bridge.md](references/pstack-bridge.md) and modes §22.
...
Follow [quality-checklist.md](references/quality-checklist.md) (incl. **Cold-agent readable** + **Plans layout** + **Vibe-proof-auditor bridge** + **Pstack plan bridge**).
...
| [references/pstack-bridge.md](references/pstack-bridge.md) | **Pstack plan bridge** (always-on bake + HITL `/architect` fold) |
...
| [references/quality-checklist.md](references/quality-checklist.md) | Done criteria (incl. **§2 Mínimo** + go/no-go + Appendix A + **production-harden DoD** + **Sólido states/transitions** + **Cold-agent readable** + **Plans layout** + **Vibe-proof-auditor bridge** + **Pstack plan bridge**) |
...
- **Silent pstack auto-run** on every plan — bake always; live `/architect` only via HITL ([modes.md §22](references/modes.md#22-pstack-plan-bridge))
